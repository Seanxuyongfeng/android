...
public class AppsCustomizeTabHost extends TabHost implements LauncherTransitionable,
	...
	if(LauncherFeature.SEARCH_WIDGET){
        AppsSearchResult searchResult = (AppsSearchResult)(findViewById(R.id.apps_search_result));
        searchResult.setAppsCustomizeContent(mAppsCustomizePane);
        MyGridLayout keyboard = (MyGridLayout)(findViewById(R.id.keyboard));
	    //keyboard.setContent(mAppsCustomizePane, searchResult);
	    searchResult.setKeyboard(keyboard);
		searchText = (EditText)findViewById(R.id.search_text);
		searchText.setInputType(InputType.TYPE_NULL);
		MyGridLayout gridLayout = (MyGridLayout)findViewById(R.id.keyboard);
		gridLayout.setEditText(searchText);
		searchResult.setEditText(searchText);
		deleteButton = (ImageView)findViewById(R.id.deleteButton);
		if(deleteButton != null){
			deleteButton.setOnClickListener(gridLayout);
		}
		
	}else{
		View v = findViewById(R.id.search_box);
		MyGridLayout gridLayout = (MyGridLayout)findViewById(R.id.keyboard);
	...
	//
        //inflate all apps indicator

        LinearLayout allapps_indicator = (LinearLayout) findViewById(R.id.paged_view_indicator);
        if(LauncherFeature.PAGE_INDICATOR_STYLE1){
		//allapps_indicator.setl
		FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams)allapps_indicator.getLayoutParams();
		lp.setMargins(0, 0, 0, /*getResources().getDimensionPixelSize(R.dimen.button_bar_height)*/40);
