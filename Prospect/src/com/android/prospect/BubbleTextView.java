...
public class BubbleTextView extends TextView {
	...
	int iconRes = AllAppsList.getSystemAppIcon(info.getClassName());
	if(iconRes != 0){
		Drawable drawable = mContext.getResources().getDrawable(iconRes);
        	BitmapDrawable bg = (BitmapDrawable)(drawable);
        	b = bg.getBitmap();
        }else{
		b = IconCache.getRandomBitmap(mContext, b);
	}


        b = scaleBitmap(b, (float)width, (float)height);
    ...
    public static Bitmap scaleBitmap(Bitmap bm, float width, float height){
        final float scaleX = width/bm.getWidth();
        final float scaleY = height/bm.getHeight();
        return Launcher.scaleBitmap(bm, scaleX, scaleY);
    }
