...
public class IconCache {
	...
	entry.icon = Utilities.createIconBitmap(
		...
		/*
	    int iconRes = AllAppsList.getSystemAppIcon(componentName.getClassName());
	    if(iconRes != 0){
		Drawable drawable = mContext.getResources().getDrawable(iconRes);
        	BitmapDrawable bg = (BitmapDrawable)(drawable);
        	entry.icon = bg.getBitmap();
            }else{
            	entry.icon = Utilities.createIconBitmap(
                    getFullResIcon(info), mContext);
		entry.icon = getRandomBitmap(mContext, entry.icon);
	    }*/
        }
        return entry;
    }

    public static Bitmap getRandomBitmap(Context context, Bitmap icon){
	assert(context != null && icon != null);
        Drawable drawable = context.getResources().getDrawable(AllAppsList.getRandomBackground());
        BitmapDrawable bg = (BitmapDrawable)(drawable);
	return Launcher.createCompoundBitmap(bg.getBitmap(), icon);
