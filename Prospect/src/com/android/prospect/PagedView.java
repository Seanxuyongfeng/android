...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    public interface PageSwitchListener {
        void onPageSwitch(View newPage, int newPageIndex, int pageCount);
    }
    ...
    public void setPageSwitchListener(PageSwitchListener pageSwitchListener) {
        ...
        if (mPageSwitchListener != null) {
            mPageSwitchListener.onPageSwitch(getPageAt(mCurrentPage), mCurrentPage, getChildCount());
        }
    }
    ...
    protected void notifyPageSwitchListener(int screen){
        if (mPageSwitchListener != null) {
            mPageSwitchListener.onPageSwitch(getPageAt(screen), screen, getChildCount());
        }
    }
	...
	if(pageCount == 1){
	    range[0] = 0;
        	...
        	//at leat one page exits
        	int x = getChildOffset(1) + getRelativeChildOffset(1);
            ...
            if(getAppPageNum() == -1){// workspace
            	
            }else{
            	if(tempX < 0){
	            	tempX += getAppPageNum() * (pageWidth + mPageSpacing);
	            }
            }
            
            while(x <= tempX && leftScreen < pageCount - 1){
            	leftScreen++;
            	x += getScaledMeasuredWidth(getPageAt(leftScreen)) + mPageSpacing;
            }
            
            rightScreen = leftScreen;
            while(x < tempX + screenWidth && rightScreen < pageCount - 1){
            	rightScreen++;
           ...
           if(getAppPageNum() == -1){//workspace
	            ...
	            if(mCurrentPage < getAppPageNum()){
		           	if((leftScreen == getAppPageNum() - 1) && (rightScreen == getAppPageNum())){
		            	rightScreen = 0;
		            }else if(leftScreen == rightScreen && rightScreen == 0){
		            	leftScreen = getAppPageNum() - 1;
		            }else if(leftScreen == getAppPageNum() - 1){//yongfeng.xu
		            	rightScreen = 0;
		            }
	            }else{//appWidgets
	            	if(leftScreen == pageCount - 1){
	            		rightScreen = getAppPageNum();
	            	}else if((rightScreen == getAppPageNum()) && (leftScreen == getAppPageNum() - 1)){
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        int screenCenter = getScrollX()/*mOverScrollX*/ + halfScreenSize;//yongfeng.xu
		...
		final boolean scaleMode =  Float.compare(mLayoutScale, 1f) != 0;
        ...
        if (pageCount > 0) {
            ...
            if (leftScreen != -1 && rightScreen != -1) {
                ...
                View child = null;
                if(LauncherFeature.PAGED_VIEW_LOOP){
			if(pageCount == 1){
 	                	drawChild(canvas, getPageAt(0), drawingTime);
			}else if(mCurrentPage < pageCount){
	                	offset = pageCount * width;
 	                	offset = getChildOffset(pageCount -1) - 
 	                			getRelativeChildOffset(pageCount -1) + 
						(getChildOffset(1) -  getRelativeChildOffset(1));	   
	                	int curPos = getChildOffset(leftScreen) - getRelativeChildOffset(leftScreen);
	 	                if(rightScreen < leftScreen){
	 	                	//temp resolution, modified by yongfeng.xu 
	 	                	//when it is scaled, draw three screens
	 	                	if(scaleMode){
	 	                		if(mScrollX >= curPos){
							child = getPageAt(leftScreen - 1);
 	                				drawChild(canvas, child, drawingTime);
	                			}else{
							child = getPageAt(rightScreen + 1);
	                				drawChild(canvas, child, drawingTime);
	                			}
	 	                	}	 	                		
	 	                	//for mScrollX > curPos, when scroll to screen 0 from the last screen,
	 	                	//modified by yongfeng.xu
		                	if(mScrollX >= curPos/*(offset - width)*/){
	                	...
	                	offset = (pageCount - getAppPageNum()) * width;
	                	if(rightScreen < leftScreen){
                    ...
                    if(rightScreen == 0 && !isScrollToRight){
                    	int offset = pageCount * screenWidth;
                    	canvas.translate(-offset, 0);
                    	drawChild(canvas, getPageAt(leftScreen),drawingTime);
                    	canvas.translate(+offset, 0);
                    }else{
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if ((isSignificantMove && deltaX > 0 && !isFling) || (isFling && velocityX > 0)) {
                	if(mCurrentPage > 0){
                		if(mCurrentPage < getAppPageNum() || getAppPageNum() == -1){
                		...
                		if(getAppPageNum() != -1){
                			finalPage = returnToOriginalPage ? mCurrentPage : getAppPageNum() - 1;
                		}else{
                	...
                	if(mCurrentPage < getChildCount() - 1){
                		if(mCurrentPage < getAppPageNum() || getAppPageNum() == -1){
                			if(mCurrentPage == getAppPageNum() - 1){
                		...
                		if(getAppPageNum() != -1){
                			if(mCurrentPage == getAppPageNum() - 1){//yongfeng.xu
                				finalPage = returnToOriginalPage ? mCurrentPage : 0;
                			}else{
                				finalPage = returnToOriginalPage ? mCurrentPage : getAppPageNum() - 1;
                			}
    ...
    int getPageNearestToCenterOfScreen() {
        ...
        if(getAppPageNum() != -1 && mCurrentPage < getAppPageNum()){
        	if(minDistanceFromScreenCenterIndex == getAppPageNum()){
        		minDistanceFromScreenCenterIndex = 0;
        	}
        }else if(getAppPageNum() != -1 && mCurrentPage >= getAppPageNum()){
        	if(minDistanceFromScreenCenterIndex == getAppPageNum() - 1){
        		minDistanceFromScreenCenterIndex = getChildCount() - 1;
        	}
        }
        if (LauncherLog.DEBUG) {
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        final int pageWidth = getScaledMeasuredWidth(getPageAt(0)) + mPageSpacing;
        int width = getWidth();
        int childCount = getChildCount();
        int offset = (getChildCount() - 1) * width;
        //replace width with pageWidth, modified by yongfeng.xu
        int AppOffset = (getAppPageNum() - 1) * pageWidth;
        if(getAppPageNum() != -1){
 	        if(mCurrentPage < getAppPageNum()){//apps
 	        	offset = AppOffset;
	        	if(mUnboundedScrollX > offset){
	        		mUnboundedScrollX = offset - mUnboundedScrollX;
	        		if(newX == 0){
	        			delta = width + mUnboundedScrollX;
	        		}
	        		mUnboundedScrollX = -delta;
	        	}else if(mUnboundedScrollX < 0){
	        		int temp = mUnboundedScrollX;
	        		 mUnboundedScrollX = offset + pageWidth + mUnboundedScrollX;
		        	 delta = -(width + temp);
	        	}
	        }else{//appWidgets
    ...
    protected void snapToPage(int whichPage, int delta, int duration) {
        ...
        if(getAppPageNum() == -1){//workspace
	        ...
	        if(!(whichPage == 0 && mScroller.getFinalX() == 0)){
	        	mScroller.startScroll(mUnboundedScrollX, 0, delta, 0, duration);
	        }
	        
	        int pageCount = getChildCount();
	        int N = getChildOffset(whichPage) - getRelativeChildOffset(whichPage);
	        if(whichPage == pageCount - 1){
		        int newX = getChildOffset(whichPage) - getRelativeChildOffset(whichPage);
		        //4 4, 604 4, 1204, 4
	        	mScroller.setFinalX(newX/*(pageCount - 1) * getScaledMeasuredWidth(getPageAt(whichPage))*/);
	        }else if(whichPage == 0){
        ...
        //Log.i(TAG, "snapToPage mCurrentPage = " + mCurrentPage);//xuyongfeng
        notifyPageSwitchListener();
        invalidate();
    }
            ...
            if (parent != null) {
                ...
                if (mHasScrollIndicator) {
                    if(LauncherFeature.PAGE_INDICATOR_STYLE1){
                    	setPageSwitchListener((PageSwitchListener)mScrollIndicator);
                    }                	
    ...
    protected void hideScrollingIndicator(boolean immediately) {
        ...
        //if (isPageMoving()) return; //bigbing
    ...
    private void updateScrollingIndicatorPosition() {
        if(mScrollIndicator == null ||
           !isScrollingIndicatorEnabled() ||
            LauncherFeature.PAGE_INDICATOR_STYLE1){
        	return;
        }
